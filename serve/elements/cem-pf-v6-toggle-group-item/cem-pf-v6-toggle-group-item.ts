...
import styles from './cem-pf-v6-toggle-group-item.css' with { type: 'css' };
 ...
 *
 * @fires cem-pf-v6-toggle-group-item-select - Fires when item is selected
 */
@customElement('cem-pf-v6-toggle-group-item')
export class PfV6ToggleGroupItem extends LitElement {
  ...
  #navigateItems(direction: number) {
    const parent = this.parentElement;
    if (!parent) return;
    const items = Array.from(parent.querySelectorAll<PfV6ToggleGroupItem>('cem-pf-v6-toggle-group-item'))
...
declare global {
  interface HTMLElementTagNameMap {
    'cem-pf-v6-toggle-group-item': PfV6ToggleGroupItem;
  }
}
