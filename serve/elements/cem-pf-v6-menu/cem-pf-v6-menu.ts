...
import styles from './cem-pf-v6-menu.css' with { type: 'css' };

/**
 * PatternFly v6 Menu component
 *
 * Container for cem-pf-v6-menu-item elements. Implements keyboard navigation
...
export class PfV6Menu extends LitElement {
  ...
  #onKeydown = (event: KeyboardEvent) => {
    ...
    if ((target as Element).tagName !== 'CEM-PF-V6-MENU-ITEM') return;
  ...
  #getMenuItems(): Element[] {
    ...
    return slot.assignedElements().filter(
      el => el.tagName === 'CEM-PF-V6-MENU-ITEM' && !(el as HTMLElement & { disabled?: boolean }).disabled
    );
  }
