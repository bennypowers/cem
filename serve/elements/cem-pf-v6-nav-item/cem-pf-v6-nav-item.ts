...
import styles from './cem-pf-v6-nav-item.css' with { type: 'css' };

/**
 * PatternFly v6 Navigation Item
 *
 * Container for navigation links with optional expandable groups.
 * The child cem-pf-v6-nav-link handles the current state independently.
 *
 * @slot - Default slot for nav-link and optional nav-group
 */
@customElement('cem-pf-v6-nav-item')
export class PfV6NavItem extends LitElement {
  ...
  #updateExpandedState() {
    ...
    this.querySelector(':scope > cem-pf-v6-nav-group')
    ...
    this.querySelector(':scope > cem-pf-v6-nav-link[expandable]')
...
declare global {
  interface HTMLElementTagNameMap {
    'cem-pf-v6-nav-item': PfV6NavItem;
  }
}
