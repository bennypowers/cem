...
// TestPathResolverHotReload_InvalidRewrites verifies that invalid URL rewrites
// are rejected during hot-reload and don't cause runtime panics
func TestPathResolverHotReload_InvalidRewrites(t *testing.T) {
	// Create in-memory filesystem with tsconfig and config file
	mfs := platform.NewMapFileSystem(nil)

	// Initial tsconfig with valid rewrites
	initialTsconfig := `{
  "compilerOptions": {
    "rootDir": "./src",
    "outDir": "./dist"
	...
	// Config file with valid rewrites initially
	validConfig := `serve:
  urlRewrites:
    - urlPattern: "/build/:path*"
      urlTemplate: "/source/{{.path}}"
`
	mfs.AddFile("/test-package/.config/cem.yaml", validConfig, 0644)
	mfs.AddFile("/test-package/package.json", `{"name": "test"}`, 0644)
	mfs.AddFile("/test-package/src/test.ts", `export class Test {}`, 0644)
	...
	// Verify initial configuration has tsconfig rewrites
	...
	if urlRewrites[0].URLPattern != "/dist/:path*" {
		t.Errorf("Expected tsconfig rewrite, got %s", urlRewrites[0].URLPattern)
	}

	// Update config file with INVALID URL rewrites (invalid template syntax)
	invalidConfig := `serve:
  urlRewrites:
    - urlPattern: "/build/:path*"
...
`
	...
	// Trigger rebuild - should reject invalid rewrites from config without panicking
	// but keep the valid tsconfig rewrites
	err = server.RebuildPathResolverForTest()
	if err != nil {
		t.Fatalf("Failed to rebuild path resolver: %v", err)
	}

	// After rebuild, should still have tsconfig rewrites (invalid config rewrites were rejected)
	urlRewritesAfter := server.URLRewrites()
	if len(urlRewritesAfter) != 1 {
		t.Errorf("Expected 1 URL rewrite (from tsconfig) after invalid config, got %d", len(urlRewritesAfter))
	}
	if len(urlRewritesAfter) > 0 && urlRewritesAfter[0].URLPattern != "/dist/:path*" {
		t.Errorf("Expected tsconfig rewrite to remain, got %s", urlRewritesAfter[0].URLPattern)
	}

	// Verify server still functions without panicking
	req := httptest.NewRequest("GET", "/dist/test.js", nil)
	rec := httptest.NewRecorder()
	server.Handler().ServeHTTP(rec, req)

	// Should get 200 OK (tsconfig rewrite still works, resolves /dist/test.js -> /src/test.ts)
	// This proves the server didn't panic and continues to function with valid tsconfig rewrites
	if rec.Code != http.StatusOK {
		t.Errorf("Expected status 200 (tsconfig rewrite still works), got %d", rec.Code)
	}
}
