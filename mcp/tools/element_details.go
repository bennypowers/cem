...
// ElementDetailsArgs represents the arguments for the element_details tool
type ElementDetailsArgs struct {
	TagName string `json:"tagName"`
	Context string `json:"context,omitempty"`
}

// handleElementDetails provides comprehensive detailed information about a custom element
func handleElementDetails(
	ctx context.Context,
	req *mcp.CallToolRequest,
	registry types.MCPContext,
) (*mcp.CallToolResult, error) {
	// Parse args from request
	args, err := ParseToolArgs[ElementDetailsArgs](req)
	if err != nil {
		return nil, err
	}
	...
	response, err := RenderTemplate("element_details", templateData)
	if err != nil {
		return nil, fmt.Errorf("failed to render element details template: %w", err)
	}
