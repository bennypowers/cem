...
func TestElementSlotsTool_Integration(t *testing.T) {
	// Test successful case with comprehensive output validation
	args := tools.ElementSlotsArgs{
		TagName: "button-element",
		Context: "integration-test",
	}

	testElementSlotsWithGolden(t, args, "integration_test.golden.md")
}

// Helper function for testing element slots with golden files
func testElementSlotsWithGolden(t *testing.T, args tools.ElementSlotsArgs, goldenFile string) {
	...
	handler := tools.MakeElementSlotsHandler(registry)
	...
	// Handle -update flag
	goldenPath := filepath.Join("../fixtures/element-slots-integration", goldenFile)
	if *update {
	...
	assert.Equal(t, string(expectedData), output, "Generated element slots should match golden file")
}