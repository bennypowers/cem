...
func TestElementEventsTool_Integration(t *testing.T) {
	// Test successful case with comprehensive output validation
	args := tools.ElementEventsArgs{
		TagName: "button-element",
		Context: "integration-test",
	}

	testElementEventsWithGolden(t, args, "integration_test.golden.md")
}

// Helper function for testing element events with golden files
func testElementEventsWithGolden(t *testing.T, args tools.ElementEventsArgs, goldenFile string) {
	...
	handler := tools.MakeElementEventsHandler(registry)
	...
	// Handle -update flag
	goldenPath := filepath.Join("../fixtures/element-events-integration", goldenFile)
	if *update {
	...
	assert.Equal(t, string(expectedData), output, "Generated element events should match golden file")
}