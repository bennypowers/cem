...
// ElementStylingArgs represents the arguments for the element_styling tool
type ElementStylingArgs struct {
	TagName string `json:"tagName"`
	Context string `json:"context,omitempty"`
}

// handleElementStyling provides detailed styling information for a custom element
func handleElementStyling(
	ctx context.Context,
	req *mcp.CallToolRequest,
	registry types.MCPContext,
) (*mcp.CallToolResult, error) {
	// Parse args from request
	args, err := ParseToolArgs[ElementStylingArgs](req)
	if err != nil {
		return nil, err
	}
	...
	response, err := RenderTemplate("element_styling", templateData)
	if err != nil {
		return nil, fmt.Errorf("failed to render element styling template: %w", err)
	}
