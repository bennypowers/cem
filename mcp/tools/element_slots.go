...
// ElementSlotsArgs represents the arguments for the element_slots tool
type ElementSlotsArgs struct {
	TagName string `json:"tagName"`
	Context string `json:"context,omitempty"`
}

// handleElementSlots provides detailed slot information for a custom element
func handleElementSlots(
	ctx context.Context,
	req *mcp.CallToolRequest,
	registry types.MCPContext,
) (*mcp.CallToolResult, error) {
	// Parse args from request
	args, err := ParseToolArgs[ElementSlotsArgs](req)
	if err != nil {
		return nil, err
	}
	...
	response, err := RenderTemplate("element_slots", templateData)
	if err != nil {
		return nil, fmt.Errorf("failed to render element slots template: %w", err)
	}
