...
func TestElementDetails_WithFixtures(t *testing.T) {
	registry := getTestRegistry(t)

	// Find all test fixture directories
	fixturesDir := "../fixtures/element-details"
	...
	for _, fixture := range fixtures {
		...
		t.Run(fixtureName, func(t *testing.T) {
			...
			var args tools.ElementDetailsArgs
			...
			handler := tools.MakeElementDetailsHandler(registry)
			...
			// General validation for successful element details
			assert.Contains(t, output, args.TagName, "Should contain element tag name")
			assert.Contains(t, output, "Complete Reference:", "Should contain complete reference header")
...
func TestElementDetailsTool_Integration(t *testing.T) {
	// Test successful case with comprehensive output validation
	args := tools.ElementDetailsArgs{
		TagName: "button-element",
		Context: "integration-test",
	}

	testElementDetailsWithGolden(t, args, "integration_test.golden.md")
}

// Helper function for testing element details with golden files
func testElementDetailsWithGolden(t *testing.T, args tools.ElementDetailsArgs, goldenFile string) {
	registry := getTestRegistry(t)
	...
	// Handle -update flag
	goldenPath := filepath.Join("../fixtures/element-details-integration", goldenFile)
	if *update {
	...
	assert.Equal(t, string(expectedData), output, "Generated element details should match golden file")
}