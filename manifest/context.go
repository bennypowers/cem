...
type PredicateFunc func(Deprecatable) bool

// Predicate: keep only deprecated nodes
func IsDeprecated(d Deprecatable) bool { return d.IsDeprecated() }
...
func (x *PackageWithContext) ToTreeNode() pterm.TreeNode {
	node := pterm.TreeNode{Text: "<root>"}
	children := make([]pterm.TreeNode, 0)
	for _, m := range x.Package.Modules {
		c := ModuleWithContext{Module: &m, Path: m.Path, Package: x.Package}
		children = append(children, c.ToTreeNode())
	}
	if len(children) > 0 {
...
func (x *ModuleWithContext) ToTreeNode() pterm.TreeNode {
	node := pterm.TreeNode{Text: x.Path}
	children := []pterm.TreeNode{}
	for _, decl := range x.Module.Declarations {
		switch d := decl.(type) {
		case *CustomElementDeclaration:
			...
			children = append(children, cedCtx.ToTreeNode())
		case *ClassDeclaration:
			classCtx := ClassWithContext{
					ClassDeclaration: d,
					Module:           x.Module,
			}
			children = append(children, classCtx.ToTreeNode())
		case *FunctionDeclaration:
			nodeText := "function " + d.Name
			children = append(children, pterm.TreeNode{Text: nodeText})
		case *VariableDeclaration:
			nodeText := "var " + d.Name
			children = append(children, pterm.TreeNode{Text: nodeText})
		case *MixinDeclaration:
			nodeText := "mixin " + d.Name
			children = append(children, pterm.TreeNode{Text: nodeText})
		case *CustomElementMixinDeclaration:
			nodeText := "custom element mixin " + d.Name
			children = append(children, pterm.TreeNode{Text: nodeText})
		}
	}
	node.Children = children
...
func (x *ClassWithContext) ToTreeNode() pterm.TreeNode {
	classDecl := x.ClassDeclaration
	node := pterm.TreeNode{Text: "class " + classDecl.Name}
	children := []pterm.TreeNode{}
	for _, member := range classDecl.Members {
		switch m := member.(type) {
		case *ClassField:
			if m.IsDeprecated() {
				children = append(children, pterm.TreeNode{Text: "field " + m.Name})
			}
		case *ClassMethod:
			if m.IsDeprecated() {
				children = append(children, pterm.TreeNode{Text: "method " + m.Name})
			}
		}
	}
	if len(children) > 0 {
...
func (x *CustomElementWithContext) ToTreeNode() pterm.TreeNode {
	ced := x.CustomElementDeclaration
	node := pterm.TreeNode{Text: "<" + x.TagName + ">"}
	children := []pterm.TreeNode{}

	// Attributes
	if len(ced.Attributes) > 0 {
		...
		for _, attr := range ced.Attributes {
			attrCtx := AttributeWithContext{
				...
				JavaScriptModule:         x.Module,
			}
			attrChildren = append(attrChildren, attrCtx.ToTreeNode())
		}
	...
	if len(ced.Events) > 0 {
		...
		for _, ev := range ced.Events {
			...
			eventChildren = append(eventChildren, evCtx.ToTreeNode())
		}
	...
	if len(ced.Slots) > 0 {
		...
		for _, slot := range ced.Slots {
			...
			slotChildren = append(slotChildren, slotCtx.ToTreeNode())
		}
		children = append(children, pterm.TreeNode{Text: "slots", Children: slotChildren})
	}

	// CSS Parts
	if len(ced.CssParts) > 0 {
		...
		for _, part := range ced.CssParts {
			...
			partChildren = append(partChildren, partCtx.ToTreeNode())
		}
		children = append(children, pterm.TreeNode{Text: "css parts", Children: partChildren})
	}

	// CSS Properties
	if len(ced.CssProperties) > 0 {
	...
	if len(ced.CssStates) > 0 {
		...
		for _, state := range ced.CssStates {
			...
			stateChildren = append(stateChildren, stateCtx.ToTreeNode())
		}
		children = append(children, pterm.TreeNode{Text: "css states", Children: stateChildren})
	}

	// Members
	if len(ced.Members) > 0 {
		memberChildren := []pterm.TreeNode{}
		for _, mem := range ced.Members {
			switch m := mem.(type) {
			case *ClassField:
				memberChildren = append(memberChildren, pterm.TreeNode{Text: "field " + m.Name})
			case *ClassMethod:
				memberChildren = append(memberChildren, pterm.TreeNode{Text: "method " + m.Name})
			case *CustomElementField:
				memberChildren = append(memberChildren, pterm.TreeNode{Text: "custom field " + m.Name})
			}
		}
...
func (x *AttributeWithContext) ToTreeNode() pterm.TreeNode {
	text := x.Name
	if x.CustomElementField != nil && x.CustomElementField.Reflects {
		text += " (reflects)"
	}
	return pterm.TreeNode{Text: text}
}
...
func (x *SlotWithContext) ToTreeNode() pterm.TreeNode {
	slotName := x.Name
	if slotName == "" {
		slotName = "<default>"
	}
...
func (x *CssCustomPropertyWithContext) ToTreeNode() pterm.TreeNode {
	return pterm.TreeNode{Text: x.Name}
}
...
func (x *CssCustomStateWithContext) ToTreeNode() pterm.TreeNode {
	node := pterm.TreeNode{Text: x.Name}
	return node
}
...
func (x *CssPartWithContext) ToTreeNode() pterm.TreeNode {
	node := pterm.TreeNode{Text: x.Name}
	return node
}
...
func (x *EventWithContext) ToTreeNode() pterm.TreeNode {
	node := pterm.TreeNode{Text: x.Name}
	return node
}
...
func (x *MethodWithContext) ToTreeNode() pterm.TreeNode {
	node := pterm.TreeNode{Text: x.Name}
	return node
}
...
// GetAllDeprecationsWithContext returns a PackageWithContext that contains only deprecated items (at any level),
// preserving context information just like GetAllDeprecations but with WithContext types at the top level.
func (x *Package) GetAllDeprecationsWithContext() (deprecations *PackageWithContext) {
	// Build a context-wrapped package
	pkgCtx := NewPackageWithContext(x)
	// Use the context tree/filter machinery to filter by deprecation and reconstruct as manifest tree
	filtered := FilterPackage(pkgCtx, IsDeprecated)
	return filtered
}
...
// --- Main API: FilterPackage ---

func FilterPackage(pkg *PackageWithContext, pred PredicateFunc) *PackageWithContext {
	rootNode := buildContextTree(pkg)
	filteredRoot := filterContextTree(rootNode, pred)
	if filteredRoot == nil {
		return nil
	}
	filteredPkg := _reconstructManifest(filteredRoot.(*PackageCtxNode))
	return NewPackageWithContext(filteredPkg)
}

// --- Private: reconstruct manifest tree from filtered context tree ---

func _reconstructManifest(pkgNode *PackageCtxNode) *Package {
	orig := pkgNode.Pkg.Package
	filtered := &Package{
		SchemaVersion: orig.SchemaVersion,
		Readme:        orig.Readme,
	}
	if orig.Deprecated != nil {
		filtered.Deprecated = orig.Deprecated
	}

	for _, modNode := range pkgNode.ChildrenNodes {
		modCtxNode, ok := modNode.(*ModuleCtxNode)
		if !ok {
			continue
		}
		origMod := modCtxNode.Mod.Module
		newMod := *origMod
		newMod.Declarations = nil
		newMod.Exports = nil

		for _, declNode := range modCtxNode.ChildrenNodes {
			switch dn := declNode.(type) {
			case *CustomElementCtxNode:
				origCed := dn.CED.CustomElementDeclaration
				newCed := *origCed
				newCed.Attributes = nil
				newCed.Events = nil
				newCed.Slots = nil
				newCed.CssParts = nil
				newCed.CssProperties = nil
				newCed.CssStates = nil
				newCed.Members = nil

				for _, node := range dn.AttrNodes {
					if leaf, ok := node.(*LeafCtxNode); ok {
						if attrCtx, ok := leaf.Item.(*AttributeWithContext); ok {
							newCed.Attributes = append(newCed.Attributes, *attrCtx.Attribute)
						}
					}
				}
				for _, node := range dn.EventNodes {
					if leaf, ok := node.(*LeafCtxNode); ok {
						if evCtx, ok := leaf.Item.(*EventWithContext); ok {
							newCed.Events = append(newCed.Events, *evCtx.Event)
						}
					}
				}
				for _, node := range dn.SlotNodes {
					if leaf, ok := node.(*LeafCtxNode); ok {
						if slotCtx, ok := leaf.Item.(*SlotWithContext); ok {
							newCed.Slots = append(newCed.Slots, *slotCtx.Slot)
						}
					}
				}
				for _, node := range dn.CssPartNodes {
					if leaf, ok := node.(*LeafCtxNode); ok {
						if partCtx, ok := leaf.Item.(*CssPartWithContext); ok {
							newCed.CssParts = append(newCed.CssParts, *partCtx.CssPart)
						}
					}
				}
				for _, node := range dn.CssPropNodes {
					if leaf, ok := node.(*LeafCtxNode); ok {
						if propCtx, ok := leaf.Item.(*CssCustomPropertyWithContext); ok {
							newCed.CssProperties = append(newCed.CssProperties, *propCtx.CssCustomProperty)
						}
					}
				}
				for _, node := range dn.CssStateNodes {
					if leaf, ok := node.(*LeafCtxNode); ok {
						if stateCtx, ok := leaf.Item.(*CssCustomStateWithContext); ok {
							newCed.CssStates = append(newCed.CssStates, *stateCtx.CssCustomState)
						}
					}
				}
				for _, node := range dn.MemberNodes {
					if leaf, ok := node.(*LeafCtxNode); ok {
						switch mem := leaf.Item.(type) {
						case ClassMember:
							newCed.Members = append(newCed.Members, mem)
						}
					}
				}
				newMod.Declarations = append(newMod.Declarations, &newCed)
			case *ClassCtxNode:
				origClass := dn.ClassCtx.ClassDeclaration
				newClass := *origClass
				newClass.Members = nil
				for _, node := range dn.MemberNodes {
					if leaf, ok := node.(*LeafCtxNode); ok {
						switch mem := leaf.Item.(type) {
						case ClassMember:
							newClass.Members = append(newClass.Members, mem)
						}
					}
				}
				newMod.Declarations = append(newMod.Declarations, &newClass)
			case *DeclCtxNode:
				newMod.Declarations = append(newMod.Declarations, dn.Decl)
			}
		}
